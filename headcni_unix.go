...
func getDelegateIPAM(n *NetConf, fenv *subnetEnv) (map[string]interface{}, error) {
	...
	// 关键修复：只添加来自YAML配置的显式路由，并检查重复
	// 这避免了重复路由的问题
	for _, route := range fenv.routes {
		if !isRouteAlreadyPresent(rtes, route) {
			rtes = append(rtes, route)
		}
	}

	// 注释掉自动添加网络路由的部分，因为这可能导致冲突
...
func doCmdAdd(args *skel.CmdArgs, n *NetConf, fenv *subnetEnv) error {
	...
	if !hasKey(n.Delegate, "ipMasq") {
		// if headcni is not doing ipmasq, we should
		ipmasq := !*fenv.ipmasq
